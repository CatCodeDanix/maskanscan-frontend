...
import type { ListingFilters, UnifiedListing } from "@/types/listing";
...
export async function queryListingsAction(filters: ListingFilters = {}) {
	try {
		...
		if (filters.hasParking) {
			conditions.push(
				sql`${scrapedListings.attributes}->>'hasParking' = 'true'`,
			);
		}
		...
		if (filters.hasStorage) {
			conditions.push(
				sql`${scrapedListings.attributes}->>'hasStorage' = 'true'`,
			);
		}
		if (filters.hasBalcony) {
			conditions.push(
				sql`${scrapedListings.attributes}->>'hasBalcony' = 'true'`,
			);
		}
